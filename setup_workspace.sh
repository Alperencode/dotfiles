  ...
  *)
  ...
  if command -v go &>/dev/null; then
    ...
    if [ "$(printf '%s\n' "$min_version" "$installed_version" | sort -V | head -n1)" = "$min_version" ]; then
      ...
      log_info "Go $installed_version is installed, but minimum version $min_version is required. Updating to $target_version..."
    fi
  fi
...
Examples:
    $0 -r repos.txt      # Install clone repos from file
EOF
}
